...
and maintain these layers yourself.

The next model was platform as a service (PaaS), which further abstracted
the operating system as well as the runtime. All you had to do was to
maintain your application.

Back in 2014, yet another level of abstraction was introduced, and that was
functions as a service (FaaS), which abstracted the application's logic
into individual functions that can be executed in response to different
events. In this model, you didn't even need a server. That was all managed
...
Edge applications will generally need both compute and state — and
API-based serverless databases are filling the state side of that equation
due to the distribution requirements. FaunaDB and PlanetScale are examples
of database platforms designed specifically for serverless architecture.

## The Future

The reduction in latency brought by Edge computing coupled with the
evolution of 5G, faster connectivity and devices, in general, can bring
latency down to previously unattainable levels — allowing a new generation
of application use cases in the near future.
