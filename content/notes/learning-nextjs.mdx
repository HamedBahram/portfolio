...
React hooks for data fetching, [SWR](https://swr.vercel.app/) was created
by the team behind NextJS. It handles caching, revalidation, focus
...
SWR will first return the data from cache, then sends the fetch request to
get the most up-to-date data. With `SWR`, components will get a stream of
data updates constantly and automatically, and the UI will be always fast
and reactive.
...
and explanations used here, where directly from
[NextJS documentation](https://nextjs.org/docs/getting-started), I strongly
