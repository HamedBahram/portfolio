...
For example if you want to pass data such as the User ID to the browser
when using **JSON Web Tokens**, you can persist the data in the token when
the `jwt` callback is called, then pass the data through to the browser in
...
```jsx {4-7} showLineNumbers title="pages/_app.js"
...
<SessionProvider
