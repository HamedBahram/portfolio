...
one options is to use `next-mdx-remote` package (a third-party package)
that allows you to load MDX data inside `getStaticProps` or
`getServerSideProps`, in the same way you would normally load any other
data in NextJS.
...
```mdx {2} showLineNumbers /sales/2-3
...
```jsx {2} showLineNumbers /name="alice"/ title="home.jsx"
...
```jsx {2} showLineNumbers /components/2-3 title="home.jsx"
import Link from 'next/link'
import Example from './example.mdx'
...
We can use this technic to add custom styles to any element, since markdown
is ultimately compiled to native HTML elements, we can create our own
custom components that map to regular HTML tags.
